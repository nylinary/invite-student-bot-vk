...
async def on_bot_added(ctx: Ctx, peer_id: int, added_by: int) -> None:
    ...
    if len(matches) != 1:
        ...
        await notify_admins(
            ...
            f"(id {peer_id}) — какой это вуз, по названию не понял.\n\n"
            f"Зайди в саму беседу и отправь там: /bind <ключ> <ссылка на беседу>\n"
            f"Ключи: {keys}…\n\n"
            f"Если беседа без вуза (общий чат) — /bindchat <название>.\n"
            f"Можно и отсюда, тогда с id: /bind {peer_id} <ключ> <ссылка>.",
        )
        return
